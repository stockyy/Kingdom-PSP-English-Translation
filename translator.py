...
def translate_json(input_json_path, api_key):
    ...
    if CHOSEN_FILE == "UI":
        ...
        print(
            f"Reduced to {len(address_list)} Japanese lines! (from {num_lines_old})"
        )
        num_iterations = math.ceil(len(address_list) / chunk_size)
            ...
            if start_index > 0:
                ...
                while True:
                    ...
                    if choice == "2":
                        start_index = 0
                        master_english_dict = {}
                        with open(OUTFILE, "w", encoding="utf-8") as wipe_fp:
                            wipe_fp.write("{}")
                        break
    ...
    for i in range(start_index, len(address_list), chunk_size):
        ...
        while attempt <= max_retries and not success:
            try:
                ...
                attempt += 1
                print(f"API error on chunk {iter_num}: {e}")
...
if __name__ == "__main__":
    ...
    if not GEMINI_API_KEY:
        ...
        find_infile()
        choose_model()
        translate_json(INFILE, GEMINI_API_KEY)
